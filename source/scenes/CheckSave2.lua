CheckSave2 = {}
class("CheckSave2").extends(NobleScene) --Endless scene has the same functionality, but different loading scene for the game
local scene = CheckSave2
local pd = playdate
...
function scene:loadGame() 
    ...
    Noble.transition(ExampleScene, nil, Noble.Transition.CrossDissolve)
end
function scene:exit() 
    Noble.Text.setFont(Noble.Text.FONT_MEDIUM)
